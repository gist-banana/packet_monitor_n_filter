...
ip_addr = str(convert_ip_to_bin((hash_addr.items()[0][1]))) # ip_saddr
while 1:
    try:
        for k in dropcnt.keys():
            ...
            if val :
                ...
                prev[i] = val
              #  print('\n *********************************************** \n')
              #  print('>> output number = ' + str(output_number))
              #  print('packet received \n' + str(delta))
                counter = counter + delta
             #   print('total packets : ' + str(counter))
             #   print('packet number : ' + str(counter) + '\n')
                ip_addr = str(convert_ip_to_bin((hash_addr.items()[0][1])))
             #   print('sender ip address : ' + str(ip_addr) + '\n')
                contents = str('num ' + str(counter) + 's_ip : ' + str(ip_addr) + 'counter : ' + str(counter))
                if counter != sent_counter:
                    ack = producer.send(topicName, contents)
                    sent_counter = counter
    # DO NOT SEND THE VALUE IF THE COUNTER VALUE IS THE SAME
                
    # IF ABOVE DOESN'T WORK, IF IP_ADDR = 0.0.0.0, DO NOT SEND
                '''
